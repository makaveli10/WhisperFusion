    ...
    def create_quantized_state_dict(self):
        ...
        for fqn, mod in self.mod.named_modules():
            if isinstance(mod, torch.nn.Linear):
                print(mod, mod.weight.shape, mod.weight.dtype)
                int8_weight, scales, _ = dynamically_quantize_per_channel(mod.weight.float(), -128, 127, torch.int8)
                cur_state_dict[f"{fqn}.weight"] = int8_weight
                cur_state_dict[f"{fqn}.scales"] = scales.to(mod.weight.dtype)
                if mod.bias is not None:
                    quantized_bias = torch.round(mod.bias / scales).to(torch.int32)
                    cur_state_dict[f"{fqn}.bias"] = quantized_bias
...
class WeightOnlyInt8Linear(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True, device=None, dtype=None):
        ...
        if bias:
            self.register_buffer("bias", torch.empty(out_features, dtype=torch.int32))
        else:
            self.register_parameter("bias", None)

    def forward(self, input: torch.Tensor) -> torch.Tensor:
        # Convert weight back to input dtype for calculation
        weight_fp = self.weight.to(dtype=input.dtype)
        output = F.linear(input, weight_fp, None)

        if self.bias is not None:
            # Convert bias to float using scales and add to output
            bias_fp = (self.bias.to(dtype=input.dtype) * self.scales).unsqueeze(0)
            output += bias_fp

        return output * self.scales.unsqueeze(0)
